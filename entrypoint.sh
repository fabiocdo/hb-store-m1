...
DEFAULT_WATCHER_EVENT_DEBOUNCE_SECONDS=5
...
use_default_if_unset WATCHER_EVENT_DEBOUNCE_SECONDS "$DEFAULT_WATCHER_EVENT_DEBOUNCE_SECONDS"
...
log_table "WATCHER_EVENT_DEBOUNCE_SECONDS" "$(format_value WATCHER_EVENT_DEBOUNCE_SECONDS "$WATCHER_EVENT_DEBOUNCE_SECONDS")"
...
if [ "$PKG_WATCHER_ENABLED" = "true" ]; then
  exec python3 -u /scripts/watcher.py \
    ...
    --watcher-event-debounce-seconds "$WATCHER_EVENT_DEBOUNCE_SECONDS" \
