...
class Watcher:
    ...
    def dry_run(self) -> list[dict]:
        """
        Plan the processing of all PKGs without applying changes.
        """
        log("info", "Planning changes...", module="WATCHER")
    ...
    def run(self):
        ...
        log("info", "Executing planned changes...", module="WATCHER")
        ...
        for item in rejected:
            ...
            if not pkg_path.exists():
                continue
            log("debug", "Moving rejected PKG to errors", message=str(pkg_path), module="WATCHER")
        ...
        for item in icon_allowed:
            ...
            log("debug", "Extracting icon", message=icon_log_path, module="WATCHER")
            pkg_key = str(pkg_path)
            if pkg_key not in sfo_cache:
                sfo_result, sfo_payload = pkg_utils.extract_pkg_data(pkg_path)
                if sfo_result != PkgUtils.ExtractResult.OK:
                    continue
                sfo_cache[pkg_key] = sfo_payload
            sfo_data = sfo_cache[pkg_key]
            content_id = sfo_data.get("content_id", "")
            if not content_id:
                continue
        ...
        for item in pkg_allowed:
            pkg_path = Path(item["source_pkg_path"])
            log("debug", "Processing PKG for format/sort", message=str(pkg_path), module="WATCHER")
            pkg_key = str(pkg_path)
            if pkg_key not in sfo_cache:
                sfo_result, sfo_payload = pkg_utils.extract_pkg_data(pkg_path)
                if sfo_result != PkgUtils.ExtractResult.OK:
        ...
        rejected_pkgs = {item["source_pkg_path"] for item in results if item["planned_pkg_output"] == "rejected"}
        skipped_pkgs = {
            item["source_pkg_path"]
            for item in results
            if item["planned_pkg_output"] == "skip" or item["planned_icon_output"] == "skip"
        }
        skipped_count = len(skipped_pkgs - rejected_pkgs)
        log(
            "info",
        ...
        for item in results:
            if item["planned_pkg_output"] == "rejected":
                continue
            pkg_key = item["source_pkg_path"]
            if pkg_key in sfo_cache:
                continue
            sfo_result, sfo_payload = pkg_utils.extract_pkg_data(Path(pkg_key))
            if sfo_result != PkgUtils.ExtractResult.OK:
                continue
            sfo_cache[pkg_key] = sfo_payload

        if self.auto_indexer_enabled:
            if "JSON" in indexer.output_formats:
                log("info", "Writing index.json...", module="WATCHER")
                indexer.run_with_cache(results, sfo_cache)
                log("info", "Index update complete", module="WATCHER")
            else:
                log("debug", "JSON output disabled. Skipping index.json", module="WATCHER")
