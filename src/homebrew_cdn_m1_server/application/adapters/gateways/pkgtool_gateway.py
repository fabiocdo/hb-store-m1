...
class PkgtoolGateway(PackageProbePort):
    ...
    @classmethod
    def _parse_sfo_entries(cls, lines: list[str]) -> dict[str, str]:
        parsed: dict[str, str] = {}
        for line in lines:
    ...
    def _version_key(cls, raw: str) -> tuple[int, ...] | None:
        ...
        if not text:
            return None
        parts = [int(item) for item in cls._VERSION_PARTS_REGEX.findall(text)]
        if not parts:
            return None
    ...
    def _extract_media(
        ...
        for entry_name, out_path, required in targets:
            ...
            if out_path.exists():
                ...
                continue
            self._run("pkg_extractentry", str(pkg_path), entry_index, str(out_path))
            extracted.append(out_path)
    ...
    def probe(self, pkg_path: Path) -> ProbeResult:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self._run("pkg_extractentry", str(pkg_path), param_index, str(sfo_path))
            sfo_raw = sfo_path.read_bytes()
            sfo_lines = self._run("sfo_listentries", str(sfo_path)).stdout.splitlines()

        fields = self._parse_sfo_entries(sfo_lines)
